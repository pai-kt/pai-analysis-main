"""data/모델학습(생육·환경·일사량) 참조 데이터 → RandomForest 학습용 테이블."""
...
SHIPMENT_RANK_PATH = DATA_ROOT / "농가별_총출하량_순위.xlsx"
MODEL_WEEKS = tuple(range(3, 13))
...
def _top_shipment_farms(n: int = 10) -> list[str]:
    """총출하량 순위 파일에서 선도 농가명을 가져옵니다."""
    ...
    if "순위" in rank.columns:
        rank = rank.sort_values("순위")
    elif "총출하량" in rank.columns:
        rank = rank.sort_values("총출하량", ascending=False)
    names = [
        unicodedata.normalize("NFC", str(value)).strip()
        for value in rank["농가명"].tolist()
    ]
...
def _list_paired_farms() -> list[tuple[str, Path, Path, str | None]]:
    ...
    top_farms = set(_top_shipment_farms(10))
    if top_farms:
        paired = [farm for farm in paired if farm[0] in top_farms]
    return paired
...
def _build_all_reference_rows(growth_features: list[str]) -> pd.DataFrame:
    from src.rolling_features import compute_rolling_summary
...
def _load_all_reference_rows(growth_key: tuple[str, ...]) -> pd.DataFrame:
    ...
    if not _cache_stale():
        try:
            ...
            cached_weeks = (
                set(pd.to_numeric(cached["_week"], errors="coerce").dropna().astype(int))
                if "_week" in cached.columns
                else set()
            )
            cached_farms = set(cached.get("_farm_key", pd.Series(dtype=str)).astype(str))
            expected_farms = {farm[0] for farm in _list_paired_farms()}
            if (
                not cached.empty
                and "_week" in cached.columns
                and cached_weeks == set(MODEL_WEEKS)
                and cached_farms <= expected_farms
            ):
...
def _slice_week_columns(df: pd.DataFrame, week: int, growth_features: list[str]) -> pd.DataFrame:
    ...
    from src.rolling_features import build_window_feature_name
...
def build_reference_week_dfs(growth_features: list[str] | None = None) -> dict[int, pd.DataFrame]:
    ...
    if all_rows.empty:
        return {wk: pd.DataFrame() for wk in MODEL_WEEKS}
    for wk in MODEL_WEEKS:
