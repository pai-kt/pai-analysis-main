...
from src.reference_training_data import (
...
def _fit_model(model_type: str, X_train, y_train):
    from src.ml_utils import make_model
...
def _predict_values(model_type: str, model, X, extra=None) -> np.ndarray:
    from src.ml_utils import safe_predict
...
def train_and_save_all_models(
    ...
    """선도 상위 10농가 데이터로 4종 모델 × 3타깃 × 3~12주 일괄 학습."""
    growth_features = growth_features or DEFAULT_GROWTH_FEATURES
    weeks = tuple(weeks) if weeks is not None else MODEL_WEEKS
    from src.reference_training_data import _list_paired_farms

    if clear_existing and MODELS_DIR.exists():
        shutil.rmtree(MODELS_DIR)
    MODELS_DIR.mkdir(parents=True, exist_ok=True)

    ref_farms = _top_shipment_farms(10)
    manifest: dict = {
        "models": [],
        "growth_features": growth_features,
        "ref_farms": len(_list_paired_farms()),
        "ref_farm_names": ref_farms,
        "selection": "농가별 총출하량 상위 10농가",
        "weeks": list(weeks),
    }
